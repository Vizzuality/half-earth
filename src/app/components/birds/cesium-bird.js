...
class CesiumBird extends Bird {
  ...
  render(clock) {
    ...
    const pos = Cesium.Cartesian3.fromDegrees(this.latScale(x), this.longScale(y));
    entity.position = pos;

    const lifeTime = clock.currentTime.secondsOfDay - clock.startTime.secondsOfDay;
    if (lifeTime <= this.burstTime) {
      entity.model.color = Cesium.Color.fromAlpha(this.color, this.alphaScale(lifeTime));
    }
    ...
    if (!this.crop) return;
    if (this.longScale(y) < south || this.longScale(y) > north || this.latScale(x) > east || this.latScale(x) < west) {
