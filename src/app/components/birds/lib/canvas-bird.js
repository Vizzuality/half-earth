...
class Bird extends Boid {
  constructor(options) {
    ...
    this.targetFactorScale = map([ minDistance, minDistanceFactor ], [ maxDistance, maxDistanceFactor ]);
  }
  setTarget(idx) {
    this.targetIdx = idx;
  ...
  render() {
    ...
    const c = `rgb(${sizeToColor(canvas.width, pos.x)}, ${sizeToColor(canvas.height, pos.y)}, 255)`;
