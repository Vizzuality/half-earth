...
class ModelProvider extends Component {
  ...
  componentWillReceiveProps({ viewer, url, coordinates, scale, animate, speed = 1 }) {
    ...
    const model = this.model = viewer.scene.primitives.add(
      Cesium.Model.fromGltf({ url, modelMatrix: modelMatrix, scale })
    );

    if (animate) {
      Cesium.when(model.readyPromise).then(function(model) {
        model.activeAnimations.addAll({ loop: Cesium.ModelAnimationLoop.REPEAT, speedup: speed });
      });
    }
  }
