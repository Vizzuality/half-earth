...
export const getCartoTiles = createThunkAction('getCartoTiles', ({ account, apiv, config, name, tileFormat }) =>
  dispatch => {
    dispatch(fetchingCartoTiles());

    return fetch(`https://${account}.carto.com/api/${apiv}/map?config=${encodeURIComponent(JSON.stringify(config))}`)
      .then(d => d.json())
      .then(({ layergroupid, cdn_url: { templates: { https: { url } } } }) => ({ layergroupid, url }))
