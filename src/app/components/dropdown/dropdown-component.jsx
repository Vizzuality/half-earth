...
const Dropdown = (
  { className, options, selected, closed, getContainer, toggleOpen, selectItem, theme, sort, ...props }
) =>
  {
    const optionKeys = sort ? keys(options).sort() : keys(options);
    return (
      <div className={cx(className, theme.dropdown)} ref={getContainer}>
        <div onClick={toggleOpen} className={cx(theme.label, { [theme.labelClosed]: closed })}>
        ...
        <ul className={cx(theme.options, { [theme.optionsClosed]: closed })}>
          {optionKeys.map((g, ...rest) => {
            return (
              <li
                key={g}
                className={cx(theme.option, { [theme.optionSelected]: g === selected })}
          ...
          {!closed && <button className={theme.closeButton} onClick={() => toggleOpen()} />}
        </ul>
      </div>
    );
  };
