...
class Knob extends Component {
  constructor(props) {
    ...
    const halfDashOffset = halfOffset * (1 - 0.5) + trackDashOffset + 2 * halfTickWidth;
  ...
  onMouseDown(e) {
    ...
    this.container = {
      center: { x: boundingBox.left + boundingBox.width / 2, y: boundingBox.top + boundingBox.height / 2 }
    };
  }

  onMouseMove(e) {
    let angle = getAngle(this.container.center.x, this.container.center.y, e.pageX, e.pageY);
    ...
    // reset going going over center
    const percent = maxClamp(lerp(angle, 0, 300, 0, 1), this.props.min || 0, this.props.max || 1);

    const state = { angle, percent };
  ...
  render() {
    ...
    const progressDashOffset = (C - (trackDashOffset + handleWidth)) * (1 - percent) + trackDashOffset + handleWidth;

    const handleDashOffset = handleOffset * (1 - percent) + trackDashOffset + 2 * handleWidth;
