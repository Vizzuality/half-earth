...
export const renderToggle = layers => toggle => (label, disabled, n, enabled = true) => {
...
export const renderDropdown = sections => selectOption => (section, name) => {
  ...
  return (
    <Dropdown
      ...
      theme={dropdownTheme}
      onSelect={layerName => selectOption({ selection: layerName, layers: sel[layerName], section, selector: name })}
      selected={selected}
    />
  );
};
