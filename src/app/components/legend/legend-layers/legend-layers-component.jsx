...
const LegendLayers = ({ layers, openPopUpLegend, popUp, closePopUp, section }) => {
  ...
  const simpleLegend = layer => (
    <span
      ...
      className={cx(styles.simpleLegend, styles['simpleLegend' + capitalize(layer.color)])}
    >
      {layer.label}
    </span>
  );
  ...
  const gradientLegend = (layer, i) => (
    <div
      ...
      className={cx({ [styles.gradientLegendSmall]: layer.size === 'small' }, styles.gradientLegend)}
    >
          ...
          {layer.label}
          {
            layer.showGroup && (
                <span className={styles.bioText}>
                  {layer.group && `Species ${capitalize(layer.group) || 'Richness'}`}
                </span>
              )
          }
        </div>
        <div className={styles.boxContainer}>
          <div className={cx([ styles.gradientBoxes, styles['gradientBoxes' + capitalize(layer.color)] ])}>
            <span>-</span>
            <span />
            <span />
            <span />
  ...
  const multipleLegend = (layer, i) => {
    ...
    const renderer = { simple: simpleLegend, gradient: gradientLegend };

    return layer.elements.map(element => renderer[element.type](element, i));
  };

  const renderLegend = (layer, i) => {
    const renderer = ({ simple: simpleLegend, gradient: gradientLegend, multiple: multipleLegend })[layer.type];
  ...
  return (
    <div>
      <div className={styles.legendLayers}>
        <button className={styles.legendModal} onClick={() => openPopUpLegend()}>
          <span>?</span>
        </button>
