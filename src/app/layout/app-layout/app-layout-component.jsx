...
const pages = { global: Global, regional: Regional };
...
const Layout = ({ isHome, route, layers, section, interaction }) => {
  ...
  return (
    <div className={cx(styles.container, { [styles.containerHover]: interaction === 'hover' })}>
      <Header className={cx(styles.header, styles.headerHidden)} />
      <div className={styles.body}>
        {
          !isHome && (
              <Sidebar route={route} hidden={isHome} className={cx(styles.col, styles.sidebar)}>
                {Component && <Component />}
              </Sidebar>
            )
        }
