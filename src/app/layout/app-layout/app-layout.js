...
function mapStateToProps(state) {
  ...
  const activeLayers = page && Array.isArray(page.layers) ? page.layers
  ...
  const groupedLayers = flatten(
    values(groupBy(activeLayers, 'group')).map(group => group.map((item, i) => ({ ...item, showGroup: i === 0 })))
  );

  const layers = groupedLayers.length > 0 && groupedLayers;
  return { page, route, isHome, layers, section, interaction: interactions.interaction };
}

export default connect(mapStateToProps, keyActions)(LayoutContainer);
