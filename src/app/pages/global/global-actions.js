...
export const setGlobalSectionThunk = createThunkAction('setGlobalSectionThunk', type => (dispatch, getState) => {
...
export const selectGlobalSelector = createAction('selectGlobalSelector', null, ({ selection }) => ({
  analytics: [ 'global', 'Change species on map', `Change to ${selection}` ]
}));
export const toggleGlobalLayer = createAction('toggleGlobalLayer', null, ({ name }) => ({
...
export const getChartData = createThunkAction('getChartData', () => (dispatch, getState) => {
  const requests = [];
  ...
  Object.keys(charts).forEach(chartName => {
    ...
    if (canonicalData) {
      req = Promise.resolve(parser(canonical)).then(chart => dispatch(setChartData({ chartName, chart })));
    } else {
      req = fetch(provider);
      req
        .then(d => d.ok ? d.json() : Promise.reject(d.statusText))
        .then(canonical => dispatch(setCanonicalData({ [provider]: canonical })) || Promise.resolve(canonical))
...
export const setTypeThunk = createThunkAction('setType:globalThunk', type => (dispatch, getState) => {
  const { section } = getState().section;
