...
//   return 0
// }
const defaultParser = (d, key) => d
  .filter(defaultFilter)
  .map(d => ({ ...d, [key]: parseFloat(d[key]) }))
  .map((d, i, l) => ({ ...d, isLast: i === l.length - 1 }));

const classifyScenarios = data => Array.isArray(data) && data.reduce((acc, next) => {
...
export default {
  globalScaleBiodiversity: {
    ...
    provider: 'https://storage.googleapis.com/cdn.mol.org/half-earth/data/reserve-coverage/all-taxa-old.json',
    key: 'totalSpecies',
  ...
  globalScaleProtectedAreas: {
    ...
    domain: [ 0, 100 ],
    color: 'purple',
    legend: 'Percentage of species for which the global protected area network provides sufficient coverage.',
    provider: 'https://storage.googleapis.com/cdn.mol.org/half-earth/data/reserve-coverage/all-taxa.json',
  ...
  globalConservationPrioritization: {
    ...
    parser: data => classifyScenarios(
      data
        .filter(d => d.taxa !== 'all')
        .map(d => ({
          ...d,
          percentSpeciesMeetingTargetProtectedAreaViaAny: parseFloat(d.percentSpeciesMeetingTargetProtectedAreaViaAny)
        }))
    )
  }
};
