...
function mapStateToProps(state) {
  ...
  const sanitizeVector = vector => vector.split(',').slice(0, 3).map(parseFloat);

  if (location.query) {
    coordinates = location.query.coordinates && sanitizeVector(location.query.coordinates);
    if (location.query.orientation) {
      const [ heading, pitch, roll ] = sanitizeVector(location.query.orientation);
      coordinatesOptions = { orientation: { roll, pitch, heading } };
    }

    if (location.query.layers) {
      const urlLayers = location.query ? location.query.layers.split(',').map(sanitizeLayerId) : [];
      activeLayers = Object.values(layers.byId).filter(layer => urlLayers.includes(layer.id) && layer.config.url);
    }
  }

  return { location, coordinates, layers: activeLayers, coordinatesOptions };
}
...
class MapLayoutContainer extends Component {
  ...
  updateMapParams = params => {
    ...
    const parsedParams = Object
      .keys(params)
      .reduce(
        (acc, next) => ({ ...acc, [next]: Array.isArray(params[next]) ? params[next].join(',') : params[next] }),
        {}
      );
    updateMapParams({ query: { ...location.query, ...parsedParams } });
  };

  render() {
    const { location, ...props } = this.props;
    return createElement(MapLayout, { ...props, updateMapParams: this.updateMapParams });
  }
}

export default connect(mapStateToProps, actions)(MapLayoutContainer);
