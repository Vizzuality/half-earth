...
const analytics = { openPopUp: [ 'Map hotspots' ] };
...
const Map = (
...
) =>
  {
    ...
    const foundRegionalPopup = _find(regional.sidePopup.content, { key: regional.sidePopup.selected });
    const [ route, zoomLevel ] = ns(routeLevel, '|');

    let [ coordinates, coordinatesOptions, camera ] = zoomLevels[zoomLevel] || zoomLevels[route];
    ...
    if (regional.sidePopup.open && foundRegionalPopup) {
      ...
      if (x && y) {
        coordinates = [ x, y, z ];
        coordinatesOptions = null;
      }
    }

    const getBillboardLayer = (billboard, layers) => _find(layers, { name: billboard.layerName });
    ...
    return (
      ...
      >
        {
          route === 'regional' && section.section === 'regional:3' && regional.billboards
              .map(billboard => ({
                ...billboard,
                show: getBillboardLayer(billboard, regional.layers) &&
              ...
              .map(billboard => (
                <Billboard
                  ...
                  {...billboard.color
                    ...
                    : {
                      color: new Cesium.Color(
                        ...(foundRegionalPopup && map.distance < foundRegionalPopup.location[2] + 5000
                          ? [ 1.0, 1.0, 1.0, 0 ]
                          : [ 1, 1, 1 ])
                      )
                    }}
                  {...billboard.distanceDisplayCondition
                    ...
                    : {}}
                  onClick={id => openSidePopup({ payload: id, meta: [ 'local', ...analytics.openPopUp, id ] })}
        ...
        {
          route === 'regional' &&
            regional.layers.map(layer => layer.url ? <CesiumLayer key={layer.name} {...layer} /> : null)
        }
        {
          route === 'global' &&
            global.layers.map(layer => layer.url ? <CesiumLayer key={layer.name} {...layer} /> : null)
        }
