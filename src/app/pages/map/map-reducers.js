...
export const updateLayer = (state, { payload, ...rest }) => {
  ...
  const filtered = difference(layers, [ layer ]).map(reset ? l => ({ ...l, visible: false }) : identity);

  return { ...state, layers: sortBy(filtered.concat([ { ...layer, ...payload(layer) } ]), 'name') };
};
...
export const toggleLayer = (state, { payload: { name } }) => {
  ...
  return isUndefined(get($exists, state)) ? state : set($currentIsVisible, !get($currentIsVisible, state), state);
};
...
export const selectLayer = (state, { payload }) =>
  updateLayer(state, { ...payload, payload: layer => ({ visible: true }) });

export default {
  updateLayer,
  [cartoActions.gotCartoTiles]: (state, { payload }) =>
    updateLayer(state, { ...payload, payload: layer => ({ url: payload.url, carto: null }) }),
