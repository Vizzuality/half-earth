        ...
        <PopUp open={regional.popup.open} close={() => closePopup()}>
          {
            regional.popup.selected &&
              <ImageContent content={{ ...regional.popup.selected, attribution: regional.popup.selected.supplier }} />
          }
        </PopUp>
        <Scroller>
          <P
            className={cx(uiStyles.slides, { [uiStyles.slidesActive]: section === 'regional:1' })}
          ...
          <P
            className={cx(uiStyles.slides, { [uiStyles.slidesActive]: section === 'regional:3' })}
            onScrollFocus={() => updateSections('regional:3')}
          >
            <span className={uiStyles.innerP}>
              While traditional reserves, such as these{' '}
              {t('Example Protected Areas')}
              , play an important role for
            conservation,{' '}
              {t('Community-based conservation areas')}
              {' '}and
              {' '}
              {t('private reserves')}
              {' '}take on a key role in this region,
            especially safe-guarding against increasing{' '}
              {t('human pressures')}
              .
            </span>
            <span className={uiStyles.innerP}>
          ...
          <P
            className={cx(uiStyles.slides, { [uiStyles.slidesActive]: section === 'regional:4' })}
