...
const toggleFilters = (state, { payload }) => {
  const current = find(state.sidePopup.content, { key: state.sidePopup.selected });
  const others = difference(state.sidePopup.content, [ current ]);
  const withFilters = { ...current, filters: [ payload ] };
  ...
  return mapReducers.hideLayers(
    mapReducers.selectLayer(
      { ...state, sidePopup: { ...state.sidePopup, content } },
...
export const setType = (state, { payload: { type, section } }) => {
  ...
  return {
    ...
    sections: {
      ...state.sections,
      [section]: { ...currentSection, selectionType: type, selections: updatedSelections }
    }
  };
};
...
export const setRegionalSection = (state, { payload: { type, section } }) => {
  ...
  const visibleLayers = layers.concat(
    selectors && Object.keys(selectors).map(s => selections[selectors[s].selected]) || []
  );

  const matchingLayers = state.layers.filter(l => includes(visibleLayers, l.name));
...
const popCloser = (state, key) => {
  return { ...state, [key]: { ...state[key], open: false, selected: null } };
};
...
export default {
  ...
  [actions.openSidePopup]: (state, { payload }) => {
    const current = find(state.sidePopup.content, { key: payload });
    const filter = first(uniq(current.species.map(f => toLower(f.taxoGroup))));
    return toggleFilters(
      { ...state, sidePopup: { ...state.sidePopup, open: true, selected: payload } },
      toPayload(filter)
    );
  },
  [actions.openPopup]: (state, { payload }) => {
    return { ...state, popup: { ...state.popup, open: true, selected: payload } };
  },
