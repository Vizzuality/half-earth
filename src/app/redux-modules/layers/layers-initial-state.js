...
const getPrioritizationLayers = range(14, 51).reduce((acc, step) => {
  ...
  const legend = legends['prioritization-of-places'];
  return {
    ...acc,
    [id]: {
      id,
      config: { name: id, opacity: 100, url: null, type: 'UrlTemplate', visible: false },
      carto: {
      ...
      legend: { ...legend, label: `${legend.label} ${step}` }
    }
  };
});
...
const createCartoLayer = (name, sql, cartocss) => ({
  ...
  legend: legends[name],
  carto: { sql, cartocss },
  config: { name, opacity: 100, url: null, type: 'UrlTemplate', visible: false }
});
...
function getSpeciesLayers() {
  ...
  return speciesGroups.reduce(
    (acc, group) => ({
      ...acc,
      ...speciesTypes.reduce(
        (acc, type) => ({ ...acc, [`${group}:${type}`]: createMolLayer(`${group}:${type}`, group, type) }),
        {}
      )
    }),
    {}
...
const layers = {
  ...
  'human-pressures': createMolLayer('human-pressures', 'esa/1km/80p', 'human-pressures'),
  'protected-areas': createMolLayer('protected-areas', 'existing-network', 'reserve-coverage')
};

export default { byId: { ...getPrioritizationLayers, ...getSpeciesLayers(), ...layers } };
