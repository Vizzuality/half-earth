export const getCartoUrl = ({ account = 'half-earth', carto = {} }) => {
  const cartoConfig = {
    version: '1.3.0',
    layers: [ { type: 'mapnik', options: { cartocss_version: '2.3.0', ...carto } } ]
  };
  return fetch(`https://${account}.carto.com/api/v1/map?config=${encodeURIComponent(JSON.stringify(cartoConfig))}`)
    .then(d => d.json())
