...
export const withReducers = ({ actions, reducers, initialState }) => Comp => class WithReducers extends Component {
  constructor(props) {
    super(props);
      ...
      bound[name] = payload => {
        ...
        if (isSymbol(actionned.key)) {
          ...
          actionned(
            ...
            { getState: () => this.state, dispatch: a => this.reduce(this.state, a) }
          );
        }
      };
      return bound;
  ...
  render() {
    return createElement(Comp, { ...this.props, ...this.state, ...this.actions });
  }
};
